    ...
    <>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div className="relative flex-1 max-w-lg">
                ...
                <input type="text" placeholder="Search by Official Name or ID..." className="pl-10 pr-10 py-2 w-full border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-transparent" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                {searchQuery && ( <button onClick={() => setSearchQuery('')} className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-gray-600"> <XIcon className="w-4 h-4" /> </button> )}
            </div>
            <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex items-center space-x-2">
                    <label className="text-sm font-medium text-gray-700">Status:</label>
                    <div className="relative">
                        <select className="pl-3 pr-8 py-2 border border-gray-300 rounded-md focus:ring-primary appearance-none bg-white" value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)}> {availableStatuses.map(status => <option key={status} value={status}>{status}</option>)} </select>
                ...
                {userRole === 'admin' && (
                    <div className="flex items-center space-x-2">
                        ...
                        <div className="relative">
                            <select className="pl-3 pr-8 py-2 border border-gray-300 rounded-md focus:ring-primary appearance-none bg-white" value={selectedAgency} onChange={(e) => setSelectedAgency(e.target.value)}> {availableAgencies.map(agency => <option key={agency} value={agency}>{agency}</option>)} </select>
                            <div className="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none"><ChevronDownIcon className="w-4 h-4 text-gray-500" /></div>
                        </div>
                    </div>
                )}
...
const AdminDashboardPage: React.FC<AdminDashboardPageProps> = ({ onLogout, userRole }) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100">
      <AdminHeader activePage={activePage} setActivePage={setActivePage} onLogout={onLogout} userRole={userRole} />
