...
const allSubmissions = [
  { id: 'DEC-2024-001', officialName: 'H.E. Lyonpo Dorji', officialId: '11223344', year: 2023, type: 'Annual', submissionDate: '2024-02-20', status: 'Pending', riskScore: 'Low', schedule: 'Schedule I', agency: 'Cabinet' },
  { id: 'DEC-2024-002', officialName: 'Dasho Pema', officialId: '99887766', year: 2023, type: 'Annual', submissionDate: '2024-02-19', status: 'Flagged', riskScore: 'High', schedule: 'Schedule I', agency: 'Ministry of Finance' },
  { id: 'DEC-2024-003', officialName: 'Mr. Tashi Wangmo', officialId: '55667788', year: 2023, type: 'Annual', submissionDate: '2024-02-18', status: 'Approved', riskScore: 'Low', schedule: 'Schedule II', agency: 'Ministry of Finance' },
  { id: 'DEC-2024-004', officialName: 'Mrs. Ugyen Tenzin', officialId: '44332211', year: 2023, type: 'Annual', submissionDate: '2024-02-15', status: 'Pending', riskScore: 'Medium', schedule: 'Schedule II', agency: 'Ministry of Finance' },
];
...
const OverviewContent = ({ onViewDetails, userRole }: { onViewDetails: (id: string) => void, userRole: UserRole }) => {
  ...
  const filteredSubmissions = allSubmissions.filter(item => {
    ...
    const matchesAgency = userRole === 'admin' || selectedAgency === 'All' || item.agency === selectedAgency;
    const matchesStatus = selectedStatus === 'All' || item.status === selectedStatus;
    return matchesSearch && matchesAgency && matchesStatus;
  });

  const availableAgencies = ['All', 'Cabinet', 'Ministry of Finance', 'Ministry of Health', 'Ministry of Education'];
  const availableStatuses = ['All', 'Submitted', 'Pending', 'Approved', 'Flagged', 'Rejected'];
  ...
  return (
    <>
        ...
        {userRole === 'admin' && (
          <div className="bg-white border border-gray-300 rounded-md p-1 flex shadow-sm">
            {['All', 'Schedule I', 'Schedule II'].map((sch) => (
              <button key={sch} onClick={() => setScheduleFilter(sch as any)}
                className={`px-4 py-1.5 text-sm font-medium rounded ${scheduleFilter === sch ? 'bg-primary text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50'}`}
              >
                {sch}
              </button>
            ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <DashboardCard title="Pending Verifications" value={filteredSubmissions.filter(i => i.status === 'Pending').length.toString()} subtitle="In current view" variant="warning" />
        <DashboardCard title="Flagged for Review" value={filteredSubmissions.filter(i => i.status === 'Flagged').length.toString()} subtitle="Requires attention" variant="danger" />
        <DashboardCard title="Reports Submitted" value={userRole === 'admin' ? "8/10" : "0"} subtitle={userRole === 'admin' ? "Agencies Filed" : "Annual Report Status"} variant="success" />
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="py-3 px-4 font-semibold text-sm text-text-secondary">Official Name</th>
                <th className="py-3 px-4 font-semibold text-sm text-text-secondary">Category</th>
                <th className="py-3 px-4 font-semibold text-sm text-text-secondary">Submitted</th>
                <th className="py-3 px-4 font-semibold text-sm text-text-secondary">Risk Score</th>
                <th className="py-3 px-4 font-semibold text-sm text-text-secondary">Status</th>
                <th className="py-3 px-4 font-semibold text-sm text-text-secondary">Action</th>
              </tr>
            </thead>
            <tbody>
              {filteredSubmissions.length > 0 ? (
                filteredSubmissions.map((item) => (
                  <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <button onClick={() => onViewDetails(item.officialId || '')} className="text-accent hover:underline font-medium text-sm">
                        View Details
                      </button>
                    </td>
                  </tr>
