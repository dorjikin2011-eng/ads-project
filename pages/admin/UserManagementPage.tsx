...
const UserManagementPage: React.FC<UserManagementPageProps> = ({ userRole }) => {
    ...
    const filteredOfficials = officials.filter(official => {
        // Role Filtering for Agency Admin
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-6">
                ...
                <div className="flex items-center gap-4 mt-4 md:mt-0">
                    ...
                    <button 
                        ...
                        className="flex items-center px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-dark transition font-medium shadow-sm"
                    >
                        <UserAddIcon />
                        ...
                        {filteredOfficials.map((official) => (
                            <tr key={official.id} className={`hover:bg-gray-50 ${official.status === 'Inactive' ? 'bg-gray-50 opacity-75' : ''}`}>
                                ...
                                <td className="py-4 px-6 text-sm text-text-secondary">
                                    <span className={`px-2 py-1 rounded-full text-xs border ${official.schedule === 'Schedule I' ? 'bg-purple-50 border-purple-200 text-purple-800' : 'bg-gray-50 border-gray-200 text-gray-700'}`}>
                                        {official.schedule}
                                    </span>
                                </td>
                                <td className="py-4 px-6">
