...
const UserManagementPage: React.FC<UserManagementPageProps> = ({ userRole }) => {
    ...
    const [roleFilter, setRoleFilter] = useState<'All' | 'agency_admin' | 'hoa'>('All'); // NEW: Role Filter
    ...
    const filteredOfficials = officials.filter(official => {
        ...
        // NEW: Role Filter for ACC Admin (e.g. View only Agency Admins)
    ...
    const promoteToAdmin = (id: string) => {
        if(confirm("Are you sure you want to promote this user to Agency Admin?")) {
    ...
    return (
        <div>
            {/* Success Modal */}
            <Modal isOpen={isSuccessModalOpen} onClose={() => setSuccessModalOpen(false)} title="User Registered Successfully">
            ...
            {/* Add User Modal (Enhanced with Role Selection for ACC) */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {userRole === 'admin' && (
                            <>
                                ...
                                {/* NEW ROLE SELECTOR */}
                                <div><label className="block text-sm font-medium text-gray-700">System Role</label><select className="mt-1 w-full px-3 py-2 border rounded-md" value={newUser.role} onChange={e => setNewUser({...newUser, role: e.target.value})}><option value="user">Declarant (Standard)</option><option value="agency_admin">Agency Admin</option><option value="hoa">Head of Agency</option></select></div>
                            </>
                        )}
                    </div>
                            ...
                            {userRole === 'admin' && (
                                <>
                                    ...
                                    <div className="bg-gray-100 rounded p-1 flex">
                                        <button onClick={() => setRoleFilter('All')} className={`text-xs px-3 py-1.5 rounded ${roleFilter === 'All' ? 'bg-white shadow text-primary font-bold' : 'text-gray-500'}`}>All Users</button>
                                        <button onClick={() => setRoleFilter('agency_admin')} className={`text-xs px-3 py-1.5 rounded ${roleFilter === 'agency_admin' ? 'bg-white shadow text-primary font-bold' : 'text-gray-500'}`}>Agency Admins</button>
                                        <button onClick={() => setRoleFilter('hoa')} className={`text-xs px-3 py-1.5 rounded ${roleFilter === 'hoa' ? 'bg-white shadow text-primary font-bold' : 'text-gray-500'}`}>HoA</button>
                                    </div>
                                    {/* SCHEDULE FILTER */}
                                    <div className="bg-gray-100 rounded p-1 flex">
                                        {['All', 'Schedule I', 'Schedule II'].map(sch => ( <button key={sch} onClick={() => setScheduleFilter(sch as any)} className={`text-xs px-3 py-1.5 rounded ${scheduleFilter === sch ? 'bg-white shadow text-primary font-bold' : 'text-gray-500'}`}>{sch === 'All' ? 'All' : sch === 'Schedule I' ? 'Sch I' : 'Sch II'}</button> ))}
                                    </div>
                                </>
                            )}
                                ...
                                {filteredOfficials.map((official) => (
                                    <tr key={official.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="py-4 px-6 text-right flex justify-end gap-2">
                                            ...
                                            {/* Promote Button for ACC Admin */}
                                            {userRole === 'admin' && !official.role && (
                                                <>
                                                    <span className="text-gray-300">|</span>
                                                    <button onClick={() => promoteToAdmin(official.id)} className="text-sm text-purple-600 font-medium hover:underline">Make Admin</button>
                                                </>
                                            )}
                                        </td>
                                    </tr>
